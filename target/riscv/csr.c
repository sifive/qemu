                                ...
                                target_ulong new_val, target_ulong wr_mask)
{
    ...
    if (!(BIT(new_mlwid) & cpu->cfg.mwidlist)) {
        if (cpu->cfg.mwidlist) {
            ...
            new_mlwid = find_first_bit(
                (unsigned long *)&cpu->cfg.mwidlist, 32);
        } else {
            /* Shouldn't be here. mwidlist could not be zero. */
            new_mlwid = 0;
        }
    }
                                ...
                                target_ulong new_val, target_ulong wr_mask)
{
    ...
    if (!env->mwiddeleg) {
        /*
         * When mwiddeleg CSR is zero, access to slwid raise an illegal
